...
t_philosophers *philosopher_init(t_program *program)
{
	...
	meal_sem = sem_open("meal_sem", O_CREAT, 0644, 1);
...
bool check_eath_enough(t_philosophers *philosopher)
{
	sem_wait(philosopher->meal_sem);
	if(philosopher->meal_count >= philosopher->program->number_of_times_each_philosopher_must_eat)
	{
		sem_post(philosopher->meal_sem);
		sem_wait(philosopher->program->stop_sem);
		philosopher->program->simulation_stopped = true;
...
void signal_other_philosophers(t_philosophers *philosopher)
{
	int	i;

	i = 0;
	while(i < philosopher->program->number_of_philosophers)
	{
		kill(philosopher->program->pids[i],SIGINT);
		i++;
	}
}
...
void *monitor_routine(void *arg)
{
	...
	while (!is_simulation_stopped(philosopher->program))
	{
		...
		{
			signal_other_philosophers(philosopher);
			return (NULL);
		}
		if (philosopher->program->number_of_times_each_philosopher_must_eat != -1 && check_eath_enough(philosopher))
			return (NULL);
		usleep(1000);
	}
	return NULL;
...
void program_destroy(t_program *program)
{
	...
	sem_close(program->philosophers[0].meal_sem);
